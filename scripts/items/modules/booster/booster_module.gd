...
func _ready() -> void:
	if GameState.has_booster_unlocked:
		queue_free()
...
func _process(delta: float) -> void:
	animate_light()
	if player_in_area and player_ref and player_ref.current_state == player_ref.States.INTERACT:
		pickup_module()
...
func pickup_module() -> void:
	...
	if player:
		...
		cutscene_instance.module_name = "Picked up the Booster Module!"
		cutscene_instance.module_description = "Allows for mid-air boosts in any direction."
